...
class OnboardingActivity : AppCompatActivity() {
    ...
    private val viewPagerListener = object : OnPageChangeListener {
        ...
        private fun setDotIndicator(position: Int) {
            dots = arrayOfNulls<TextView?>(5) // Adjusted for 5 screens
            dotIndicator.removeAllViews()
            for (i in dots.indices) {
                dots[i] = TextView(this@OnboardingActivity) // Explicitly reference the outer class context
                dots[i]?.text = Html.fromHtml("&#8226;")
                dots[i]?.textSize = 35f
                dots[i]?.setTextColor(ContextCompat.getColor(this@OnboardingActivity, R.color.desc)) // Also explicitly reference the outer class context
                dotIndicator.addView(dots[i])
            }
            dots[position]?.setTextColor(ContextCompat.getColor(this@OnboardingActivity, R.color.fonts)) // Explicitly reference the outer class context
        }
